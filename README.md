...
| **WebArena** | Browser-based web tasks (Reddit/Postmill starter subset) | 10 curated (full) / 8 (smoke) | Playwright + accessibility-tree actions via `PromptAgent` |
...
## Quick start: WebArena (smoke mode — no Docker, no 50 GB download)

The fastest way to try the loop on a browser-based benchmark. A tiny local Flask app stands in for WebArena's Postmill site so real Playwright + real agent + real evaluators all run end-to-end in under a minute, without pulling the 50 GB Postmill image.

**Requirements:** an `OPENAI_API_KEY`, Playwright Chromium installed, and a coding agent.
...
# 2. Install Python deps (webarena extra pulls in playwright, beartype, nltk, ...)
...
# 4. Configure the experiment
cp experiment_config.yaml.template experiment_config.yaml
# edit experiment_config.yaml — uncomment the "WebArena Smoke Mode" block

# 5. Start the local fixture in a separate terminal (keep it running)
python fixtures/webarena_smoke/app.py

# 6. Initialize workspace + run baseline (uses the 8-task smoke split)
python prepare.py

# 7. Start the optimization loop
# Point your coding agent at the repo and prompt:
#   "Read PROGRAM.md and start the optimization loop."
```

Smoke mode ships 8 hand-authored WebArena-shaped tasks (`webarena_data/smoke_tasks/10001-10008.json`) split 5 train / 3 test, all pure info-seeking with `string_match` evaluators. Tasks 10006–10008 require clicking into a post detail page and are designed to fail on the baseline, giving the optimization loop real signal to improve against. Flip `smoke: false` in `experiment_config.yaml` to switch to the full WebArena dataset once you have the Postmill container running — see the `WEBARENA` section of `experiment_config.yaml.template` for the tarball-download instructions.
...
└── webarena.py            # webarena agent starting point (subclasses PromptAgent)

program_templates/
├── tau_bench.md           # tau-bench PROGRAM.md
├── terminal_bench.md      # terminal-bench PROGRAM.md
└── webarena.md            # webarena PROGRAM.md
```
...
benchmark.py                benchmark execution layer (abstract + tau-bench + terminal-bench + webarena)
webarena/
  driver.py                 per-task subprocess orchestrator for webarena (Playwright + evaluator)
fixtures/webarena_smoke/    local Flask fixture used by webarena smoke mode
webarena_data/
  task_split.json           train/test split (generated by prepare.py)
  smoke_tasks/              8 smoke-mode task configs (IDs 10001-10008)
  reddit_candidate_tasks.json  curated full-mode task IDs
