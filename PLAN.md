...
- [~] **M3 — Real sandbox execution.** Code **built** + task subset selected. `HarborExecutor` shells out to harbor 0.21.0 (`-a <module:Class>`, per-job candidate under `agent/_candidates/`, per-job jobs-dir → parses reward + trace excerpt; **no `benchmark.py` change needed**). Verified: agent API compatible with harbor 0.21.0; executor imports/constructs. **Pending: one real E2B run to validate** (needs `OPENAI_API_KEY` + `E2B_API_KEY` in `.env` + budget go-ahead; smoke=3 tasks first, then core=12). `HarborExecutor` runs a **candidate** agent in an **E2B** sandbox, captures real per-task trace/stdout → LLM context. Executor selectable per job. Handle sandbox failure → job `failed` + `None` rewards. Harness integration per §4b.
