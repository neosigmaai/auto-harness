...
def test_run_service_marks_real_run_failed_when_runner_returns_no_results():
    class EmptyRunner:
        def run(self, task_ids, *, model, sandbox_provider, requested_concurrency, run_id):
            return {}

    store = FakeStore()
    service = RunService(store=store, terminal_runner=EmptyRunner(), max_local_concurrency=2)
    request = RunCreateRequest(
    ...
    assert any(result.error_summary == "Task result missing from runner output" for result in results.task_results)


def test_run_service_serializes_real_runs_with_a_process_local_semaphore():
    import threading
    ...
    run = service.submit_run(request, org_id="org-1", created_by="user-1", start_background=False)
    run_2 = service.submit_run(request, org_id="org-1", created_by="user-1", start_background=False)

    first = threading.Thread(target=service.execute_run, kwargs={"run_id": run.run_id, "org_id": "org-1"})
    second = threading.Thread(target=service.execute_run, kwargs={"run_id": run_2.run_id, "org_id": "org-1"})

    first.start()
    assert runner.first_started.wait(timeout=1)
    second.start()

    assert not runner.second_started.wait(timeout=0.1)
